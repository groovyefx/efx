...
import efx.form.validation.annotation.RegexValidator;
 ...
 *
 */
public class SimpleRegexValidator implements AnnotationValidator {
	...
	public boolean validate(AbstractModel model, Annotation annotation, Object fieldValue, String fieldName) {
		...
		if (fieldValue instanceof String) {
			...
			RegexValidator anno = (RegexValidator) annotation;			
			if(!value.matches(anno.regex())){
				if (StringUtil.isNotEmpty(anno.message())) {
					...
					model.addFieldError(fieldName, "格式不正确");
				}
				return false;
			}
