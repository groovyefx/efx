...
public class PersonFormController extends FormController {
	...
	protected void init() {
		person = new Person();
		def options = FXCollections.observableArrayList()
		options<<new Option('M','男')
		options<<new Option('F','女')
		gender.items = options		
	}
		
	public void commit(){
		person.checkValid()		
		println person.gender().value
	}
